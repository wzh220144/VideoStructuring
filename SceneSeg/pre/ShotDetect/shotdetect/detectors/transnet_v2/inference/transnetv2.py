...
class TransNetV2:
    ...
    def __init__(self, model_dir=None):
        if model_dir is None:
            model_dir = os.path.join(os.path.dirname(__file__), "transnetv2_model/")
            if not os.path.isdir(model_dir):
                raise FileNotFoundError(f"[transnet_v2] ERROR: {model_dir} is not a directory.")
            else:
                print(f"[transnet_v2] Using weights from {model_dir}.")

        self._input_size = (27, 48, 3)
        try:
            self._model = tf.saved_model.load(model_dir)
        except OSError as exc:
            raise IOError(f"[transnet_v2] It seems that files in {model_dir} are corrupted or missing. "
    ...
    def predict_raw(self, frames: np.ndarray):
        assert len(frames.shape) == 5 and frames.shape[2:] == self._input_size, \
            "[transnet_v2] Input shape must be [batch, frames, height, width, 3]."
    ...
    def predict_frames(self, frames: np.ndarray):
        assert len(frames.shape) == 4 and frames.shape[1:] == self._input_size, \
            "[transnet_v2] Input shape must be [frames, height, width, 3]."
        ...
        for inp in input_iterator():
            ...
            print("\r[transnet_v2] Processing video frames {}/{}".format(
    ...
    def predict_video(self, video_fn: str):
        ...
        print("[transnet_v2] Extracting frames from {}".format(video_fn))
...
def main():
    ...
    for file in args.files:
        if os.path.exists(file + ".predictions.txt") or os.path.exists(file + ".scenes.txt"):
            print(f"[transnet_v2] {file}.predictions.txt or {file}.scenes.txt already exists. "
        ...
        if args.visualize:
            if os.path.exists(file + ".vis.png"):
                print(f"[transnet_v2] {file}.vis.png already exists. "
