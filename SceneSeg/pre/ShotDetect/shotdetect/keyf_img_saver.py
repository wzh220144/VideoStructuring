...
def generate_images(video_manager, shot_list, output_dir, num_images=3,
    ...
    if num_images == 1:
        for i, (start_time, end_time) in enumerate(shot_list):
            ...
            t = start_time + int(duration.get_frames() / 2)
            timecode_list[i].append(t)
            ts.add(t.get_frames())
            cnt += 1
    else:
        ...
        for i, (start_time, end_time) in enumerate(shot_list):
            ...
            if middle_images > 0:
                ...
                for j in range(middle_images):
                    t = start_time + ((j+1) * duration_increment)
                    timecode_list[i].append(t)
                    ts.add(t.get_frames())
                    cnt += 1
    ...
    for i in timecode_list:
        for j, image_timecode in enumerate(timecode_list[i]):
            ret_val = False
            if pre_frame == image_timecode.get_frames():
                frame_im = pre_frame_im
                ret_val = True
            else:
            ...
            if ret_val:
                pre_frame_im = frame_im
                pre_frame = image_timecode.get_frames()
                cv2.imwrite(get_output_file_path(
