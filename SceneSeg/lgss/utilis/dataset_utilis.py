...
def get_demo_scene_list(cfg,pred_list, video_name):
    anno_dict = {}
    ...
    # print(pair_dict)
    shotfrm_fn = osp.join(cfg.shot_frm_path,"{}.txt".format(video_name))
...
def pred2scene(cfg, threshold, video_name, logs_dir):
    ...
    print('...scene dict with the information of each scene is saved in {}'.format(osp.join(logs_dir,"pred_scene.json")))
...
#     source_movie_fn = '{}.mp4'.format(osp.join(cfg.data_root, "video", cfg.video_name))
    ...
    out_video_dir_fn = args.output_root
